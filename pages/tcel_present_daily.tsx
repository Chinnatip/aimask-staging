...
import { tcelData } from '../components/static/aimask_static'
import { readTCELCSV } from '../components/strategy/daily'
...
const Page = () => {
  ...
  useEffect(() => {
    (async () => {
      //
      const location: any = await readTCELCSV('export_location')
      const [ r, ...rows ] = location.data
      console.log(r)
      let objects : MarkerType[] = []
      parseLocation( rows, objects )
      setMarker(objects)
      setData(tcelData(objects))
      //
      // Collect report
      const report: any = await readTCELCSV('initial')
    ...
    <div className="flex-grow w-full p-4 flex">
      <div className="h-full w-3/5 relative mr-4">
        <div className="absolute top-0 left-0 flex w-3/5 bg-gray-300 z-10 text-center text-lg shadow-2xl">
          <span className="w-4/5 p-3 pt-4" >แผนที่เเสดง <span className="font-bold">การใส่หน้ากากอนามัย</span> <br/> โดย TCELs ทั้งหมด</span>
          <span className="w-1/5 p-2 border-l border-gray-400 font-bold "> <div className="text-3xl -mb-2">{camera}</div>จุด</span>
        </div>
        {/* Google Map */}
        <GooglemapComponent markers={markers} zoom={16} useStyle={false}/>
        {/* District Box */}
        <div className="absolute bottom-0 left-0 w-full mb-10 h-16 flex px-4">
          { sort_district.red.length > 0 && <div className="h-full rounded-2xl w-2/5 bg-red-600 p-4 flex text-white">
            <div className="w-2/3 flex items-center text-md font-semibold text-center">
              {sort_district.red.map((d,index) => `${index > 0 ?', ':''}${d}`)}
            </div>
